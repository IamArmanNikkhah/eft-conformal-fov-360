...
def getData(user_id, dataset_name="avtrack360"):
    ...
    # ---------- NEW PATH (CLEANED DATA) ----------
    user_json_path = os.path.join(CLEANED_DIR, f"{user_id}.json")

    if not os.path.exists(user_json_path):
        print(f"[ERROR] Cleaned JSON not found: {user_json_path}")
        return pd.DataFrame()
    ...
    with open(user_json_path, "r") as f:
        data = json.load(f)
        rows = []
        print(f"[DEBUG] getData() loading from: {os.path.abspath(user_json_path)}")

        for entry in data["data"]:
            video_id = entry.get("filename", "")  
